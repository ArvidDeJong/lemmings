...
- Every refusal is the same `abort(404)`: no configured token, no token given, a wrong one, an array in `?token[]=`. A 403 or a message for "wrong token" tells a visitor that there is a route and a secret to guess, and whether a token is configured.
