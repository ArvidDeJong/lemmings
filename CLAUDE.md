...
- Every refusal is the same `abort(404)`: no configured token, no token given, a wrong one, an array in `?token[]=`. A 403 or a message for "wrong token" tells a visitor whether a token is configured and that a guess was wrong. The 404 does not hide that the path exists: the throttle adds `X-RateLimit-*` headers to it, which a 404 for an unknown path does not have. Don't document it as hidden.
- Keep `throttle:5,1` on the route. Without it the token can be guessed at full speed.
